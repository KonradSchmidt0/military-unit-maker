...
import { GetChildIdFromPath, getComplexChildList } from "../../logic/Units/childGetting";
...
function TreeView(p : TreeViewProps) {
  ...
  const classification = GetFoldingClassification(
    p.path, p.leftDisplayDepth, echelonFoldingLevel, unitMap, trueRootId, foldingUnfoldingMap, actingRootPath, stacking,
    phase
  )

  function getDP(path: number[], startingIndex: number, count: number) {
    return getMergedDPFromChildren(path, startingIndex, count, unitMap, trueRootId, staffNames, staffComments, phase)
  }
    ...
    <div className={p.parentClassification === "b" ? "relative flex flex-row" : "relative flex flex-col"}>
      ...
      <div className={"tree-" + classification}>
        {unit.type === "org" && classification !== "c" && 
          getComplexChildList(unit, !stacking, phase).map((entry, i) => (
            <TreeView
...
export function GetFoldingClassification(
  ...
  foldingMap: FoldingMap, actingRootPath: number[], isTreeStacking: boolean, phase: number
): "a" | "b" | "c" {
  const id = GetChildIdFromPath(trueRootId, path, unitMap, phase) as string
  ...
  // Reached base value
  const complexChildrenList = getComplexChildList(unit as OrgUnit, !isTreeStacking, phase)
  if (unit.type === "raw" || complexChildrenList.length === 0) {
    return "c"
  }
  ...
  for (const entry of complexChildrenList) {
    const foldClass = GetFoldingClassification(
      [...path, entry.flatIndex], depthLeft - 1, minEchelonLvl, unitMap, 
      trueRootId, foldingMap, actingRootPath, isTreeStacking, phase
    )
