...
export function HoverInspector() {
  ...
  const eqGroups = useEquipGroupingStore((s) => s.groups)
  const { id, pos, simple, setPos } = useHoverStore();
  ...
  return (
    <div
      ref={tooltipRef}
      className={`editor-box pointer-events-none !max-w-2xl !w-fit !absolute !z-10 dark:!bg-bg !bg-white !border-r-2 rounded-lg ${
        show ? " transition-opacity opacity-100 duration-300" : "opacity-0"
      }`}
      style={{ top: pos.top, left: pos.left }}
    >
      <div className="flex flex-col px-2 gap-2 py-2">
        <h1 className="font-bold text-center text-lg">{header}</h1>
        {desc && <h2>{desc}</h2>}
        <ul className="grid grid-cols-3 gap-x-2 gap-y-1">
          {id &&
            getGroupedEquipmentTable(id, unitMap, eqGroups, phase).map(({name, count, type: _, group}) => {
              const style = group ? {color: group.color} : undefined;
              return (
                <li 
                  key={name} 
                  className="border border-bg/30 dark:border-white/20 rounded py-1 border-dashed px-2"
                  style={style}
                >
                  <b>{name}</b>: {count}
                </li>
